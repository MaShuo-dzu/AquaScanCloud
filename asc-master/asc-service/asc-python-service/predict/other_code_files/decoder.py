...
class APFormerHead(nn.Module):
    def __init__(self, module=ConvModule, **kwargs):
        super().__init__()

        self.in_index = kwargs['in_index']
        self.input_transform = kwargs['input_transform']
        self.align_corners = kwargs['align_corners']

        c1_in_channels, c2_in_channels, c3_in_channels, c4_in_channels = kwargs['in_channels']
        tot_channels = sum(kwargs['in_channels'])
...
class APFormerHeadMulti(nn.Module):
    """
    暂时不能用
    """
...
class APFormerHeadSingle(nn.Module):  # single key + replacement
    """
    Attention-Pooling Former
    """
